...
(val dimensions: Int, val strides: Array[Int])(implicit state: argon.State, pev: argon.Type[FixPt[FALSE, Log2Size, _0]],
  ...
  def log2(x: Int): Int = {
    ((0 to (x - 1)) find {1 << _ >= x}).get
  }

  def PrefixSumHillisSteele(vals: Array[ParameterIndex]): Array[ParameterIndex] = {
    val current = vals.clone
    (0 to log2(vals.length)) foreach {
      i => {
        (0 to vals.length) foreach {
          j => {
            if (j < (1 << i)) {
            } else {
              current(j) = current(j) + current(j - (1 << i))
            }
          }
        }
      }
